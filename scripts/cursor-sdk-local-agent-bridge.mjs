...
let listenHost = process.env.CURSOR_SDK_BRIDGE_HOST || "127.0.0.1";
let listenPort = parseInteger(process.env.CURSOR_SDK_BRIDGE_PORT, 8792);
...
function startServer() {
  ...
  server.listen(listenPort, listenHost, () => {
    ...
    if (address && typeof address === "object") {
      ...
      if (address.address && address.address !== "::") listenHost = address.address;
    }
    console.log(`Cursor SDK local-agent bridge listening on http://${listenHost}:${listenPort}/sdk`);
  });
  return server;
}

function sdkPublicUrl() {
  return `http://${listenHost}:${listenPort}/sdk`;
}

async function rebindListen(nextHost, nextPort) {
  const host = String(nextHost || "").trim();
  const port = Number(nextPort);
  if (!host) throw new HttpError("Host is required", 400, "invalid_host");
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new HttpError("Port must be an integer between 1 and 65535", 400, "invalid_port");
  }
  if (host === listenHost && port === listenPort) return { host: listenHost, port: listenPort, url: sdkPublicUrl() };
  if (!server) {
    listenHost = host;
    listenPort = port;
    return { host: listenHost, port: listenPort, url: sdkPublicUrl() };
  }
  ...
  console.log(`Cursor SDK local-agent bridge listening on ${sdkPublicUrl()}`);
  return { host: listenHost, port: listenPort, url: sdkPublicUrl() };
}
...
async function handleRequest(request, response) {
  ...
  if (request.method === "POST" && url.pathname === "/listen") {
    ...
    const nextHost = String(body.host || "").trim();
    const nextPort = Number(body.port);
    if (!nextHost) throw new HttpError("Host is required", 400, "invalid_host");
    ...
    writeJson(response, {
      ...
      host: nextHost,
      port: nextPort,
      url: `http://${nextHost}:${nextPort}/sdk`
    });
    setImmediate(() => {
      rebindListen(nextHost, nextPort).catch((error) => {
        console.error("[sdk-bridge] listen rebind failed", error);
      });
    });
    return;
  }
...
async function runLocalAgentBody(input, onRun, onEvent) {
    ...
    if (!capturedToolCall) {
      for await (const event of run.stream()) {
        if (event.type === "status") {
          if ((event.status === "ERROR" || event.status === "error") && typeof event.message === "string" && event.message.trim()) {
  ...
  if (result.status === "error") {
    ...
    throw sdkRunFailureError(result, streamError);
  }
...
function localAgentCreateOptions(input) {
  return {
    ...
    local: {
      cwd: sdkWorkingDirectory(input.workingDirectory)
    }
  };
}
...
function clientForwardingMcpServers(clientTools = [], cacheKey = "") {
  return {
    [clientMcpServerName]: {
      ...
      env: {
        CURSOR_SDK_BRIDGE_CALLBACK_URL: `http://${listenHost}:${listenPort}${clientToolCallbackPath}`,
...
function sdkWorkingDirectory(value) {
  ...
  return path.isAbsolute(trimmed) ? trimmed : path.resolve(defaultCwd, trimmed);
}
...
function sdkRunFailureError(result, streamError = "") {
  const summary = sdkRunFailureSummary(result, streamError);
  ...
  error.cause = summary;
  console.warn(`Cursor SDK run returned error status${summary.code ? ` (${summary.code})` : ""}${summary.message ? `: ${summary.message}` : ""}.`);
  return error;
}

function sdkRunFailureSummary(result, streamError = "") {
  ...
  const code = firstNonEmptyString(source?.code, result?.code, result?.errorCode);
  return {
    status: result?.status,
    code,
    message,
    retryable: isRetryableSDKRunError(source) || isRetryableSDKRunError({ message, code }) || (!message && !code)
  };
}
...
// Prefer whatever the error actually says. A 401 raised by this bridge itself
// (a bad bridge token) must not be relabelled as a Cursor credential failure:
// that sends the operator hunting for a new API key instead of fixing config.
