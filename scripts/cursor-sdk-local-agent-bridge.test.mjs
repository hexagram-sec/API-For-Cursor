...
describe("Cursor SDK local-agent bridge", () => {
  ...
  it("treats opaque SDK error results as retryable but preserves explicit auth failures", () => {
    ...
    expect(sdkRunFailureSummary({ status: "error" }, "cwd must be an absolute path")).toMatchObject({
  ...
  it("keeps a bridge token failure distinct from a Cursor credential failure", () => {
    // Both are 401s, but they need opposite fixes: one is local config, the
    // other is a new API key. Collapsing them costs the operator real time.
    const error = Object.assign(new Error("Invalid bridge token"), { status: 401, code: "unauthorized" });

    expect(openAiError(error).error.message).toBe("Invalid bridge token");
    expect(openAiError(error).error.status).toBe(401);
  });
  ...
  it("resolves a relative working directory to an absolute path for the SDK", () => {
    expect(localAgentCreateOptions({ apiKey: "test", model: "composer-2.5", workingDirectory: "." }).local.cwd).toBe(process.cwd());
    expect(localAgentCreateOptions({ apiKey: "test", model: "composer-2.5", workingDirectory: "./" }).local.cwd).toBe(process.cwd());
    expect(path.isAbsolute(localAgentCreateOptions({ apiKey: "test", model: "composer-2.5", workingDirectory: "tmp" }).local.cwd)).toBe(true);
  });
  ...
  it("reports host/port/url on /health and rebinds via POST /listen", async () => {
    const port = 18000 + Math.floor(Math.random() * 1000);
    const nextPort = port === 18999 ? 18000 : port + 1;
    ...
    try {
      ...
      expect(health.url).toBe(`http://127.0.0.1:${port}/sdk`);

      const listenResponse = await fetch(`http://127.0.0.1:${port}/listen`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ host: "127.0.0.1", port: nextPort })
      });
      expect(listenResponse.ok).toBe(true);
      const rebound = await waitForJson(`http://127.0.0.1:${nextPort}/health`);
      expect(rebound.port).toBe(nextPort);
      expect(rebound.url).toBe(`http://127.0.0.1:${nextPort}/sdk`);
    } finally {
      child.kill();
    }
  });
