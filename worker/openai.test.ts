...
describe("OpenAI compatibility adapter", () => {
  it("maps Cursor model items into an OpenAI model list and injects the SDK harness alias", () => {
    const listed = modelListFromCursor(
      [
        { id: "default", displayName: "Auto" },
        { id: "composer-2.5", displayName: "Composer 2.5" },
        { id: "claude-opus-5", displayName: "Claude Opus 5" }
      ],
      { sdk: true, opencode: true }
    ) as { data: Array<{ id: string; name: string }> };

    expect(listed.data.map((model) => model.id)).toEqual([
      "default",
      "composer-2.5",
      "composer-2.5-sdk",
      "claude-opus-5"
    ]);
    expect(listed.data.find((model) => model.id === "composer-2.5")?.name).toBe("Composer 2.5");
    expect(listed.data.find((model) => model.id === "composer-2.5-sdk")?.name).toBe("Composer 2.5 SDK Harness");
    expect(listed.data.find((model) => model.id === "claude-opus-5")?.name).toBe("Claude Opus 5");
  });
