...
export function modelList(options: { opencode?: boolean; sdk?: boolean } = {}): Record<string, unknown> {
  return modelListFromCursor(FALLBACK_CURSOR_MODELS, options);
}

export function modelListFromCursor(
  items: Array<{ id?: string; displayName?: string }> | undefined,
  options: { opencode?: boolean; sdk?: boolean } = {}
): Record<string, unknown> {
  const data = (items ?? [])
    .filter((item): item is { id: string; displayName?: string } => typeof item?.id === "string" && Boolean(item.id.trim()))
    .map((item) => {
      const id = item.id.trim();
      return modelItem(id, displayNameForModel(id, item.displayName, options));
    });
  if (options.sdk && !data.some((model) => model.id === "composer-2.5-sdk")) {
    const composerIndex = data.findIndex((model) => model.id === "composer-2.5");
    data.splice(composerIndex < 0 ? 0 : composerIndex + 1, 0, modelItem("composer-2.5-sdk", "Composer 2.5 SDK Harness"));
  }
...
const FALLBACK_CURSOR_MODELS = [
  ...
  { id: "composer-latest", displayName: "Composer latest alias" },
  { id: "gpt-5.3-codex", displayName: "Codex 5.3" },
  { id: "gpt-5.2-codex", displayName: "Codex 5.2" },
  { id: "gpt-5.1-codex-max", displayName: "Codex 5.1 Max" },
...
function displayNameForModel(id: string, displayName: string | undefined, options: { opencode?: boolean }): string {
  if (options.opencode && id === "composer-2.5") return "Composer 2.5";
  const named = displayName?.trim();
  if (named) return named;
  if (id === "composer-2.5") return options.opencode ? "Composer 2.5" : "Cursor Composer 2.5";
  return id;
}
